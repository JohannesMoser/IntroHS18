...
uint8_t PID_GetPIDConfig(PID_ConfigType type, PID_Config **confP) {
	...
	case PID_CONFIG_POS_LEFT:
		*confP = &config.posLeftConfig;
		break;
	case PID_CONFIG_POS_RIGHT:
		*confP = &config.posRightConfig;
		break;
	case PID_CONFIG_SPEED_LEFT:
		*confP = &config.speedLeftConfig;
		break;
	case PID_CONFIG_SPEED_RIGHT:
		*confP = &config.speedRightConfig;
		break;
	default:
		*confP = NULL;
		return ERR_FAILED;
...
static void PID_SpeedCfg(int32_t currSpeed, int32_t setSpeed, bool isLeft,
		PID_Config *config) {
	int32_t speed;
...
static void PID_PosCfg(int32_t currPos, int32_t setPos, bool isLeft,
		PID_Config *config) {
	int32_t speed, val;
	MOT_Direction direction = MOT_DIR_FORWARD;
	MOT_MotorDevice *motHandle;
...
static void PID_PrintHelp(const CLS1_StdIOType *io) {
	...
	CLS1_SendHelpStr((unsigned char*) "  fw speed <value>",
			(unsigned char*) "Maximum speed % value\r\n", io->stdOut);
#if PL_CONFIG_HAS_CONFIG_NVM
	CLS1_SendHelpStr((unsigned char*) "  store",
			(unsigned char*) "Store PID configuration settings in FLASH\r\n",
			io->stdOut);
	CLS1_SendHelpStr((unsigned char*) "  load",
			(unsigned char*) "Load PID configuration settings from FLASH\r\n",
			io->stdOut);
#endif
}
...
void PID_Init(void) {
	...
	config.posRightConfig.maxSpeedPercent =
			config.posLeftConfig.maxSpeedPercent;
}
