...
char *CLS1_fgets(char *buf, int bufSize, void *channel) {
	...
	*buf = '\0';
	for (;;) {
		...
		foundLine = CLS1_ReadLine(buf, buf + len, bufSize - len,
				CLS1_GetStdio());
		if (foundLine) {
...
unsigned CLS1_fprintf(void *stream, const char *fmt, ...) {
	...
	va_start(args, fmt);
	count = XF1_xvformat(CLS1_printfPutChar, (void*) CLS1_GetStdio()->stdOut,
			fmt, args);
	va_end(args);
	return count;
}
...
uint8_t ZORK_ParseCommand(const uint8_t *cmd, bool *handled,
		CLS1_ConstStdIOType *io) {
	if (UTIL1_strcmp((char*)cmd, CLS1_CMD_HELP) == 0
			|| UTIL1_strcmp((char*)cmd, "ZORK help") == 0) {
		CLS1_SendHelpStr((unsigned char*) "ZORK",
				(const unsigned char*) "Group of ZORK commands\r\n",
				io->stdOut);
